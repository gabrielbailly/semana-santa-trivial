...
function buildRoundQuestions(allQuestions, difficulty, selectedCategories = []) {
  ...
  const targetCount = selectedCategories.length === 1 ? 6 : 12;
  ...
  if (selected.length < targetCount) {
    ...
    const fallbackPool = allQuestions.filter((q) => {
      const correctDifficulty = Number(q.difficulty) === Number(difficulty);
      const correctCategory =
        selectedCategories.length > 0
          ? selectedCategories.includes(q.category)
          : CATEGORY_ORDER.includes(q.category);

      return correctDifficulty && correctCategory && !selectedIds.has(q.id);
    });

    const extra = shuffle(fallbackPool).slice(0, targetCount - selected.length);
...
export default function App() {
  ...
  useEffect(() => {
    ...
    setProgress(stored);
    setHasProgress(stored.totalScore > 0 || stored.history.length > 0);
  }, []);
  ...
  const rankingTop10 = useMemo(() => {
    return [...savedScores]
      .sort((a, b) => (b.roundScore ?? b.score ?? 0) - (a.roundScore ?? a.score ?? 0))
      .slice(0, 10);
  }, [savedScores]);
  ...
  const startRound = (keepProgress) => {
    ...
    setHasProgress(
      nextProgress.totalScore > 0 || (nextProgress.history?.length ?? 0) > 0
    );
  ...
  const applyAnswer = (isCorrect) => {
    ...
    setHasProgress(nextProgress.totalScore > 0 || nextProgress.history.length > 0);
  };
      ...
      {screen === "home" && (
        <div className="card">
          ...
          <div style={{ display: "flex", gap: 12, marginTop: 18, flexWrap: "wrap" }}>
            {!hasProgress ? (
              <button className="btn btnPrimary" onClick={() => startRound(false)}>
                Jugar
              </button>
            ) : (
              <>
            ...
            {rankingTop10.length === 0 ? (
              ...
              rankingTop10.map((entry, idx) => (
                <div key={`${entry.id}_rank`} className="scoreItem">
                  ...
                  <span>{entry.roundScore ?? entry.score}</span>
                </div>
              ))
            )}
          </div>
